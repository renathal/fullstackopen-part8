...
const resolvers = {
  Query: {
    ...
    allAuthors: () => {
      const authorDict = {};
      authors.forEach((a) => {
        authorDict[a.name] = { ...a, bookCount: 0 };
      });

      books.forEach(({ author }) => {
        authorDict[author].bookCount += 1;
      });
      console.log(authorDict);

      const authorList = [];
      for (let author in authorDict) authorList.push({ ...authorDict[author] });
      return authorList;
    },
  },
  Mutation: {
